...
add_action('admin_menu', 'xero_admin_actions');
function xero_admin_actions() // Function to create left menu on wp-admin
{
	if (function_exists('add_menu_page')) {
		
	  add_menu_page('Page title', 'Xero Sync', 'manage_options', 'xero-invoice/xero-invoice.php', 'xero_settings_menu');
	  add_submenu_page( 'xero-invoice/xero-invoice.php', 'Page title', 'Xero Sync History', 'manage_options', 'xero-invoice/invoice_history.php', 'invoice_history');
	  add_submenu_page( 'xero-invoice/xero-invoice.php', 'Page title', 'Help', 'manage_options', 'xero-invoice/plugin_help.php', 'plugin_help');
	}
...
function invoice_history() // function to display invoice history
{
	...
	$current_url = get_site_url().'/wp-admin/admin.php?page=xero-invoice/invoice_history.php'; // Select base url of history page
	...
	<form action="<?=get_site_url().'/wp-admin/admin.php'?>" method="get" class="history_form">
		...
		<input type="text" id="start_date" value = <?=$start_date?> name="start_date" placeholder="Start date">
		<b style="font-size:15px">To</b>
		<input type="text" id="end_date" value = <?=$end_date?> name="end_date" placeholder="End date">
	...
	if(!empty($results))
	{
	echo pagination($totalposts,$p,$lpm1,$prev,$next,$current_url);
   ?>
   <style>
   .invoice_list td{
   border : 1px solid;
...
register_activation_hook( __FILE__, 'xero_create_table' );
function xero_settings_menu()
{
	...
	$redirect_url = get_site_url().'/wp-admin/admin.php?page=xero-invoice/xero-invoice.php';
	if(isset($_POST['save_data']))
	{
		$application_key = $_POST['application_key'];
		$secret_key = $_POST['secret_key'];
		//$redirect_url = $_POST['redirect_url'];
		$target_dir = WP_PLUGIN_DIR . '/xero-invoice/xero_library/certs/';
	...
	if($application_key != '' && $secret_key != '' && $redirect_url != '' && isset($_SESSION ['auth_button_clicked']) && $_SESSION ['auth_button_clicked'] == 1)
	{
		...
		//include WP_PLUGIN_DIR."/xero-invoice/xero_library/tests/testRunner.php";
		$signatures = array (
		...
		if (XRO_APP_TYPE == "Private" || XRO_APP_TYPE == "Partner") {
			$signatures ['rsa_private_key'] = WP_PLUGIN_DIR . '/xero-invoice/xero_library/certs/privatekey.pem';
			$signatures ['rsa_public_key'] = WP_PLUGIN_DIR . '/xero-invoice/xero_library/certs/publickey.cer';
		}
		...
		// you could handle any config errors here, or keep on truckin if you like to live dangerously
			...
			if (isset ( $oauthSession ['oauth_token'] )) {
				...
				include WP_PLUGIN_DIR . '/xero-invoice/xero_library/tests/tests.php';
		...
		<form action="" method="post" enctype="multipart/form-data">
			<div class="xero_credentials">
				<div class="input_fields" style="clear:left;">
					...
					<input type="text" name="application_key" value="<?=$application_key?>" style="width:350px;" required  >
				...
				<div class="input_fields" style="clear:left;">
					...
					<input type="text" name="secret_key" value="<?=$secret_key?>" style="width:350px;" required>
				</div>
				<div class="input_fields" style="clear:left;">
...
function add_xero_invoice_to_account( )
{
	...
	if ( isset( $_REQUEST['wpsc_action'] ) && ($_REQUEST['wpsc_action'] == 'submit_checkout') ) {
		...
		$xml = "<Invoices>
			...
			//$myfile = fopen(WP_PLUGIN_DIR."/xero-invoice/XerSyncLog.txt", "w") or die("Unable to open file!");
...
add_action( 'wpsc_submit_checkout', 'add_xero_invoice_to_account' );

function pagination($totalposts,$p,$lpm1,$prev,$next,$current_url){
    $adjacents = 3;
    if($totalposts > 1)
    {
	...
	<div class="help_content" style="font-size: 15px;">
		...
		Xero Sync is an extension of WP eCommerce plugin. This plugin syncs Invoices for all purchases in the WP eCommerce plugin to Xero. For syncing with Xero, this plugin needs a <a href="http://developer.xero.com/documentation/getting-started/private-applications">Xero private application</a> to be registered in the user's Xero Account.</p>
			...
			<ol>
				...
				<li>Click 'Add Application' to open application setup page <img class="main_page_img" src="<?=plugins_url(). '/xero-invoice/images/set_up_plugin.png'?>" style="margin: 15px 0px; border: 1px solid #000;width: 86%;">
				...
				<li>Private application will be created will be redirected to the application detail page <img class="application_detalis_img" src="<?=plugins_url(). '/xero-invoice/images/application_detalis.png'?>" style="margin: 15px 0px; border: 1px solid #000;width: 86%;" ></li>
